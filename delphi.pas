...
uses
  ...
  Datasnap.Xmlxform, Xml.XMLIntf, Xml.XMLDoc, IdIOHandler, IdIOHandlerStream;
...
begin
  try
  begin
    Json       := '{"email" : "consulta@enterplug.com.br", "password" : "consulta"}';
    JsonToSend := TStringStream.Create(UTF8Encode(Json));

    sConsultaToken := 'http://186.249.34.34/auth';
    ...
    try
      ...
      try
        sResultadoToken := IdHTTP.Post(sConsultaToken, JsonToSend);
      except
        on E: Exception do
        begin
          raise Exception.Create('Não foi possivel realizar a consulta, motivo: ' + e.Message);
        end;
      end;
    finally
      JsonToSend.Free()
    ...
    // Montando a string de consulta com o token baixado
    sConsulta := url;
...
//                 '&documentoConsumidor=' + documento;
    ...
    IdHTTP.Request.CustomHeaders.AddValue('Authorization', 'Bearer ' + sResultadoToken);
    ...
    // Baixando o arquivo
    try
      sResultado := IdHTTP.Post(sConsulta, JsonToSend);
    except
      on E: Exception do
      begin
        raise Exception.Create('Não foi possivel realizar a consulta, motivo: ' + e.Message);
